...
pipeline {
    ...
    stages {
        stage('Cloning Git') {
            steps {
                checkout([$class: 'GitSCM', branches: [[name: '*/master']], doGenerateSubmoduleConfigurations: false, extensions: [], submoduleCfg: [], userRemoteConfigs: [[credentialsId: '', url: 'https://github.com/RisathR/New-Assignment.git']]])
            }
        }
     ...
     steps{
         script {
             sshagent(credentials : ['upgrad']){
                sh 'ssh -o StrictHostKeyChecking=no ubuntu@10.0.2.228 "docker login -u AWS -p $(aws ecr get-login-password --region us-east-1) 201733685594.dkr.ecr.us-east-1.amazonaws.com/assignment && docker pull 201733685594.dkr.ecr.us-east-1.amazonaws.com/assignment:latest && (docker ps -f name=node -q | xargs --no-run-if-empty docker container stop) && (docker container ls -a -fname=node -q | xargs -r docker container rm) && docker run -d -p 8081:8081 --rm --name node 201733685594.dkr.ecr.us-east-1.amazonaws.com/assignment"'
